...
def broadcast_message(
    ...
    Args:
        ...
        session_pattern: 会话名称模式（如 "master_*", "*_task_*"）
        message_type: 消息类型
        priority: 消息优先级
        sender_session: 发送者会话
    """
    try:
        # 确定目标会话列表
        if target_sessions:
            ...
            targets = _match_session_pattern(list(all_sessions.keys()), session_pattern)
        else:
            # 广播给所有会话
            all_sessions = _session_registry.list_all_sessions()
            targets = list(all_sessions.keys())
        ...
        for session_name in targets:
            try:
                ...
                message = {
                    "id": str(uuid.uuid4()),
                    "timestamp": datetime.now().isoformat(),
                    "sender": sender_session or "broadcast_system",
                    "recipient": session_name,
                    "type": message_type,
                    "priority": priority,
                    "content": message_content,
        ...
        result = {
            ...
            "broadcast_summary": {
                ...
                "failed_sends": len(failed_sessions)
            },
            "message_details": {
                "content": message_content,
                "type": message_type,
